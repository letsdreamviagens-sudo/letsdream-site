  ...
  try {
    ...
    if (!rateKey) {
      return res.status(400).json({
        error: "rooms[0].rateKey é obrigatório"
      });
    }

    if (!body?.holder?.name || !body?.holder?.surname) {
      return res.status(400).json({
        error: "holder.name e holder.surname são obrigatórios"
      });
    }

    if (
      !Array.isArray(body?.rooms?.[0]?.paxes) ||
      body.rooms[0].paxes.length === 0
    ) {
      return res.status(400).json({
        error: "rooms[0].paxes deve ter pelo menos 1 hóspede"
      });
    }
    ...
    if (!API_KEY || !SECRET) {
      return res.status(500).json({
        error: "Faltam HOTELBEDS_API_KEY / HOTELBEDS_SECRET"
      });
    }
