...
function setRoomImage(){
  ...
  else if(hotelMeta.includes("maceio") || hotelMeta.includes("maceió")) img="/img/alagoas.jpg";
...
function renderRoomDetails(){
  ...
  document.getElementById("roomHotelMeta").textContent =
    `${hotelName} • ${hotelMeta}`;
  ...
  document.getElementById("roomDetails").innerHTML = `
    ...
    <div class="rate-card">
      ...
      <div class="rate-left">
        ...
        <b>${escapeHtml(roomName)}</b><br/>

        <div class="muted">
        Regime: ${escapeHtml(board)}
        </div>

        <div class="muted">
        Hotel: ${escapeHtml(hotelName)}
        </div>
      ...
      <div class="rate-right">
        ...
        <div class="price">
        ${escapeHtml(currency)} ${net.toFixed(2)}
        </div>
...
async function confirmRoomBooking(){
  ...
  try{
    ...
    const detailsTxt =
      JSON.stringify(e?.data?.details || e?.data || {}, null, 2);
